...
- load the upstream structured module design document from `StageRunContext.inputArtifacts`
...
ImplementationGenerator -> ImplementationPromptBuilder: build(module_design_doc, workplan, current_step, project_context)
...
- `StageRunContext.inputArtifacts["module_design_document"]`
...
```ts
interface ModuleDesignDocumentInput {
  moduleName: string
  content: string
}
```
...
- `user_prompt` contains the structured module design document, accepted workplan, current step, and relevant project context.
...
- step review result decides whether workflow advances to the next step, retries the current step, or stops
