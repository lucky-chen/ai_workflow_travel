...
export async function runImplementationStageRunnerTests() {
...
async function testImplementationStageRunnerApply(generator, contractChecker, workspaceRoot) {
...
async function testImplementationStageRunnerReject(generator, contractChecker, workspaceRoot) {
    ...
    await assert.rejects(rejectRunner.run(createRunContext("task-2", workspaceRoot)), /Change review ended with action "reject"/);
...
async function testImplementationStageRunnerContractFailure(generator, workspaceRoot) {
    const failingGate = new InMemoryChangeGate();
    const failingContractChecker = {
        async check(_context, _output) {
            return {
                passed: false,
    ...
    await resetWorkspace(workspaceRoot);
    await assert.rejects(failingRunner.run(createRunContext("task-3", workspaceRoot)), /Implementation contract failed: Contract failed before apply\./);
    assert.equal(failingGate.getLastRequest(), undefined);
    await assert.rejects(access(path.join(workspaceRoot, "src", "generated.ts")));
}
function createGenerator(artifactStore) {
...
function createRunContext(taskId, workspaceRoot) {
    return {
        taskId,
